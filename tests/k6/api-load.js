...
export function setup() {
  ...
  }), { headers: { 'Content-Type': 'application/json', 'Host': `${TENANT}.${__ENV.APP_DOMAIN || 'eduflow.app'}` } });
...
export default function (data) {
  const headers = {
    ...
    'Host': `${TENANT}.${__ENV.APP_DOMAIN || 'eduflow.app'}`,
  };
  ...
  for (const endpoint of endpoints) {
    ...
    const ok = check(res, { [`${endpoint} status 200 or 400`]: (r) => r.status === 200 || r.status === 400 });
    errorRate.add(!ok);
  }

...
function SetBudgetModal({ open, onOpenChange, costCenters }) {
  ...
  const [form, setForm] = useState({
    costCenterId: '', period: 'monthly', cap: '', year: String(new Date().getFullYear()),
  });
  ...
  const mutation = useMutation({
    mutationFn: (data) => api.post('/expense/budgets', { ...data, cap: Number(data.cap), year: Number(data.year) }).then((r) => r.data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['budgets'] });
      onOpenChange(false);
      setForm({ costCenterId: '', period: 'monthly', cap: '', year: String(new Date().getFullYear()) });
      setError('');
    },
      ...
      <DialogContent className="sm:max-w-md">
        ...
        >
          <div className="flex flex-col gap-1.5">
            ...
            <select value={form.costCenterId} onChange={update('costCenterId')} required className={selectCls}>
          ...
          <Input label="Cap Amount (INR)" type="number" value={form.cap} onChange={update('cap')} required min="1" />
        ...
        {budgets.map((b) => {
          ...
          return (
            <TableRow key={b._id} className="bg-card">
              ...
              <TableCell className="px-4 py-3 text-muted-foreground capitalize">{b.period}</TableCell>
              <TableCell className="px-4 py-3">{b.cap?.toFixed(2)}</TableCell>
              <TableCell className="px-4 py-3">{b.spent?.toFixed(2)}</TableCell>
              <TableCell className={`px-4 py-3 font-medium ${remaining < 0 ? 'text-destructive' : 'text-emerald-600 dark:text-emerald-400'}`}>
                {remaining.toFixed(2)}
              </TableCell>
